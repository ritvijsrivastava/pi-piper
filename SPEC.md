...
6. Keep the "no public hosting, Tailscale-only, defense-in-depth"
...
└───────────────────────────┘   │      └───────────────────────────┘   │
                    ...
                    │  /ws?session=<id>  (tailscale-auth, RPC relay)      │
                    │  /ws/control       (tailscale-auth, list/status push)│
                    │  /api/sessions     (tailscale-auth, HTTP snapshot)  │
                    │  /            (PWA static assets)                   │
                    └───────────────────────┬────────────────────────────┘
                                            │ wss (tailscale serve)
                                            ▼
...
```
src/
  config.rs           # + Hub-level config: bind, agent token path
  ...
  server/
    ...
    auth.rs              # + agent-token check, Tailscale identity header check
...
Connect: `wss://<hub>/ws/control`, authorized by the `Tailscale-User-Login`
header (see §7.1) — no token in the URL. Purely Hub-authored; no
...
One shared secret, plus a network-layer check, for two different
reasons:

| Mechanism | Purpose | Where it lives | Who sees it |
|---|---|---|---|
| `Tailscale-User-Login` identity header | Authorizes `/ws`, `/ws/control`, `/api/sessions` | Stamped by `tailscaled` onto every request it proxies in via `tailscale serve` | Nothing to configure or store — not a secret at all |
...
- Exposed to the tailnet exclusively via `tailscale serve` (never
  `tailscale funnel`).
- Both mechanisms are defense-in-depth on top of Tailscale's
  network-level access control, not the primary boundary.

### 7.1 Phone-facing routes: any tailnet device is authorized
...
`/ws`, `/ws/control`, and `/api/sessions` authorize a request purely on
the presence of a non-empty `Tailscale-User-Login` header, **without**
checking it against an allowlist — any tailnet login is accepted.
There is no separate phone token or per-login configuration anymore:
Piper's access-control boundary for these routes is "reachable via
`tailscale serve` at all," which is exactly what Tailscale ACLs (who
can resolve/connect to this node) already govern. Requests carrying
`Tailscale-Funnel-Request` are always rejected regardless of login, as
a belt-and-suspenders measure given Piper's Funnel-never posture above.

**Caveat, and why `/agent` doesn't work the same way:** the Hub trusts
this header at face value. That's only sound because it binds to
loopback and is meant to be reached exclusively through `tailscale
serve`'s proxy — but unlike a real reverse-proxy setup with a Unix
socket only `tailscaled` can write to, Piper has no way to
cryptographically distinguish a connection `tailscaled` actually
proxied in from any other local process on the same machine that opens
a loopback connection and sets the same header itself. So every local
user on the Hub's machine must already be trusted with full control of
your `pi` sessions (true of most single-user desktops, the primary
target here) — and `/agent` specifically still needs its own secret
(`PIPER_AGENT_TOKEN`) rather than trusting this header, because
`piper-agent` connects directly over loopback and never goes through
`tailscale serve` at all, so there's no header to check there in the
first place. The heavier alternative that closes the local-process gap
— making the Hub its own tailnet node via `tsnet` instead of sitting
